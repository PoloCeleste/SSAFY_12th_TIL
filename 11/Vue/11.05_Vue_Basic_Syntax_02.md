...
- 반응형 데이터를 포함한 로직의 경우 computed를 활용하여 미리 값을 계산하여 계산된 값을 사용
...
- **`'v-else'`** directive를 사용하여 **`v-if`**에 대한 **`else`** 블록을 나타낼 수 있음
  ...
  <script>const isSeen = ref(true)</script>
...
```vue
<script>const name = ref('Cathy')</script>

<div v-if="name === 'Alice'">Alice입니다.</div>
<div v-else-if="name === 'Bella'">Bella입니다.</div>
<div v-else-if="name === 'Cathy'">Cathy입니다.</div>
<div v-else>내 아입니다</div>
```
...
```vue
<script>const isShow = ref(false)</script>
...
#### 내장 특수 속성 [`'key'`](https://vuejs.org/api/built-in-special-attributes.html#key)
...
##### 동일 요소에 `v-for`과 `v-if`를 함께 사용하지 않는다
...
- **`v-if`**가 더 높은 우선 순위를 가지므로 **`v-for`** 범위의 **`todo`** 데이터를 **`v-if`**에서 사용할 수 없음

<img src="./assets/v-for_v-if_error.png" alt="image-20241105101451089" style="zoom: 50%;" />
...
- 첫 번째 인자 (**`source`**)
  ...
  - watch가 감시하는 대상 (반응형 함수, 값을 반환하는 함수 등)
...
```vue
<input v-model="message"> <p>Message length: {{ message Length }}</p>
<script>
	const message = ref('')
...
<script>
	watch([foo, bar], ([newFoo, newBar], [prevFoo, prevBar]) => {
    	/* ... */
        // {deep:true} 원래는 깊이 1만 감시하지만,
        // 내부 깊이가 어떻든 모두 감시 가능하도록 옵션 추가 가능
    })
</script>
```
...
| **사용 목적** | 계산된 값을 캐싱하여 재사용 중복 계산 방지 |             데이터 변화에 따른 특정 작업을 수행              |
...
#### [Lifecycle Hooks Diagram](https://vuejs.org/guide/essentials/lifecycle.html#lifecycle-diagram)

- 컴포넌트의 생애 주기 중간 중간에 함수를 제공

➤ 개발자는 컴포넌트의 특정 시점에 원하는 로직을 실행할 수 있음

<img src="./assets/lifecycle.MuZLBFAS.png" alt="Component lifecycle diagram" style="zoom: 50%;" />
...
#### 주요 [Lifecycle Hooks](https://vuejs.org/api/composition-api-lifecycle.html)
...
- **`computed`**에서 **`reverse()`** 및 **`sort()`** 사용시 원본 배열을 변경하기 때문에 원본 배열의 복사본을 만들어서 진행 해야 함
